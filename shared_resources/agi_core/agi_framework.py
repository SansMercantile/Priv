...
from shared_resources.utils.personality_loader import get_personality_loader, PersonalityProfile
...
class AGIState:
    ...
    personality: Optional[PersonalityProfile] = None  # Added personality profile

class AGICore(ABC):
    """Base class for AGI-enabled agents with advanced capabilities"""
    
    def __init__(self, agent_id: str, broker: MessageBrokerInterface, system_name: Optional[str] = None):
        self.agent_id = agent_id
        self.broker = broker
        
        # Load personality for this system
        self.system_name = system_name or self._infer_system_name(agent_id)
        self.personality_loader = get_personality_loader(self.system_name)
        ...
        self._log_personality_context()
    
    def _infer_system_name(self, agent_id: str) -> str:
        """Infer system name from agent_id"""
        # Extract system name from agent_id (e.g., "priv_agent" -> "PRIV")
        if "_" in agent_id:
            return agent_id.split("_")[0].upper()
        return agent_id.upper()
    
    def _log_personality_context(self):
        """Log personality context on initialization"""
        if self.personality_loader.is_loaded:
            logger.info(f"🎭 {self.personality_loader.get_identity_context()}")
        else:
            logger.warning(f"⚠️  {self.system_name}: Operating without personality context")
